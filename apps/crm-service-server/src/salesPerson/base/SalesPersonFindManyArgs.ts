/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { SalesPersonWhereInput } from "./SalesPersonWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { SalesPersonOrderByInput } from "./SalesPersonOrderByInput";

@ArgsType()
class SalesPersonFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => SalesPersonWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => SalesPersonWhereInput, { nullable: true })
  @Type(() => SalesPersonWhereInput)
  where?: SalesPersonWhereInput;

  @ApiProperty({
    required: false,
    type: [SalesPersonOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [SalesPersonOrderByInput], { nullable: true })
  @Type(() => SalesPersonOrderByInput)
  orderBy?: Array<SalesPersonOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { SalesPersonFindManyArgs as SalesPersonFindManyArgs };
